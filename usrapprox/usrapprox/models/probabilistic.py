...
def calculate_logits(n: torch.Tensor, linear=False):
    ...
    if linear:
        listen_prob = torch.full_like(n, 0.5)  # listen: more likely as n -> 1
        skip_prob = torch.zeros_like(n)  # skip: more likely as n -> 0
        ...
        like_prob = (1 / (1 + torch.exp(-10 * (n - 0.6)))) * 0.8
        dislike_prob = (1 - 1 / (1 + torch.exp(-10 * (n - (1 - 0.6))))) * 0.8
        love_prob = 1 / (1 + torch.exp(-(1.4444 + 10 * (n - 0.85))))
        hate_prob = 1 - 1 / (1 + torch.exp(0.44439 - 10 * (n - 0.25)))

    result = torch.stack(
        (listen_prob, skip_prob, like_prob, dislike_prob, love_prob, hate_prob), dim=-1
    )
    return result
