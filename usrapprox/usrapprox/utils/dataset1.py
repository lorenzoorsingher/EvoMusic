...
class ContrDatasetMERT1(Dataset):
    ...
    def _load_embedding(self, key):
        ...
        """
        if key in ContrDatasetMERT1.embeddings: return None  # Skip if already loaded
    ...
    def _preload_embeddings(self):
        ...
        for key, emb in results:
            if emb is not None:
                ContrDatasetMERT1.embeddings[key] = emb
        print(f"[DATASET] Preloaded {len(ContrDatasetMERT1.embeddings)} embeddings out of {len(self.emb_keys)}")
    ...
    def __getitem__(self, idx):
        ...
        if self.preload:
            ...
            poslist = [ContrDatasetMERT1.embeddings[posset]]
        else:
        ...
        for neg in negset:
            if self.preload:
                neg_emb = ContrDatasetMERT1.embeddings[neg]
