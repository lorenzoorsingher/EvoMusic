...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained("m-a-p/MERT-v1-95M", trust_remote_code=True).to(
        DEVICE
    )
    ...
    for idx, track in enumerate(dataloader):
        ...
        for i in range(audio.size(0)):
            ...
            inputs = feature_extractor(
                input_audio, sampling_rate=24000, return_tensors="pt", padding=True
            )
        ...
        print(
            f"Processed {idx + 1}/{len(dataloader)} batches in {(end_time - start_time):.2f} seconds (mean: {mean_time:.2f})"
        )
        remaining_time = (len(dataloader) - idx - 1) * mean_time
        print(
            f"Estimated time remaining: {remaining_time/60/60:.0f}h {remaining_time/60%60:.0f}m {remaining_time%60:.0f}s"
        )
        start_time = time.time()
